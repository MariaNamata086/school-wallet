...
const [errorMessage, setErrorMessage] = useState<null | string>(null);
  return (
   <main className='bg-primary h-auto items-center justify-center p-20 tracking-wider leading-loose'>
    ...
    <FormInput type='email' value={userEmail} placeholder='enter your email' onChange={(event) => setUserEmail(event.target.value)}
    required
    //find icon
    />
    <Button  loading={loading} textColor='text-primary' disabled={loading}>Submit</Button> 
    <div className='flex items-center gap-[2px]'>
        <div className='h-1 bg-slate-700'/>
        <span className=''>Or</span>
        <div className='h-1 bg-slate-700'/>
        </div> 
        <Link href='/authentication/login' className=''>Back to Login</Link> 
   </main>
  )
}
